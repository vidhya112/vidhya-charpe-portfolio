          ...
          <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2 overflow-x-auto pb-2 lg:pb-0 scrollbar-none">
              <button
                ...
                className={`px-4 py-2 text-xs font-mono rounded-xl transition-all duration-200 shrink-0 flex items-center gap-1.5 ${
                  activeCategory === 'all'
                    ? 'bg-brand-violet text-white font-bold shadow-glow-sm border border-brand-cyan/40'
            ...
            <div className="relative w-full lg:w-72 shrink-0">
              ...
              <input
                type="text"
                placeholder="Search skills (e.g. Redux, Jest)..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full pl-10 pr-9 py-2.5 text-xs font-mono bg-surface-elevated/90 border border-white/10 rounded-xl text-white placeholder-slate-400 focus:outline-none focus:border-brand-violet/70 transition-all shadow-inner"
              />
              {searchQuery && (
                <button
        ...
        {filteredCategories.length === 0 ? (
          <div className="py-16 text-center rounded-3xl glass-panel border border-white/10 space-y-4">
            <p className="text-slate-400 font-mono text-sm">
              No skills found matching "<span className="text-white">{searchQuery}</span>"
            </p>
            <button
              onClick={() => {
                setSearchQuery('');
              ...
              className="px-4 py-2 rounded-xl text-xs font-mono text-brand-sky bg-brand-violet/20 hover:bg-brand-violet/40 border border-brand-violet/30 transition-all"
            >
              Reset Filters
            </button>
          </div>
